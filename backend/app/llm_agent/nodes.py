...
from .tools.tools import fetch_all_market_data, fetch_general_market_news
from .memory import save_query, watchlist, get_recent_queries
from .prompts import (
...
User query: {query}"""
        ...
        # Log intent analysis results with new logger
        agent_logger.log_intent_analysis(
            query=query,
            intents=[{
                ...
                "watchlist_action": intent.watchlist_action
            } for intent in intents],
            processing_time_ms=duration_ms
        )

        # Generate research checklist for research intents
        from .state import ChecklistItem
        research_checklist = []
...
async def node_watchlist_executor(state: MarketState) -> MarketState:
    ...
    from .tools.watchlist_tools_supabase import handle_watchlist_command
    ...
    if not user_id:
        logger.error("❌ user_id not found in state for watchlist operation")
        state.summary = "Unable to access watchlist: user ID not available."
        state.error = "user_id missing"
        return state
    ...
    try:
        ...
        # Call watchlist command with user_id and db_manager
...
async def node_parallel_fetcher(state: MarketState) -> MarketState:
    ...
    try:
        ...
        if "news" in tools:
            ...
            from .tools.tools import AlphaIntelligence, fetch_alphavantage_news
...
async def node_web_research(state: MarketState) -> MarketState:
    ...
    try:
        ...
        from .tools.web_research import research_loop
    ...
    **NEW: Uses checklist-based parallel query execution**
    ...
    try:
        ...
        if state.research_checklist and len(state.research_checklist) > 0:
            ...
            from .tools.general_research import parallel_query_research
            ...
            from .tools.general_research import general_research
